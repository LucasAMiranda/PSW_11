...
def cadastrar_empresa(request):
    if request.method == "GET":
        ...
        try:
            #instancia em memória todos os dados
            empresa = Empresas(
            user=request.user, #Usuário precisa está logado, senão Anonymous User
            nome=nome,
            cnpj=cnpj,
            site=site,
            ...
            empresa.save() #Salva no banco de dados 
        except:
            messages.add_message(request, constants.ERROR, 'Erro interno do sistema') #Erro tipo 500
